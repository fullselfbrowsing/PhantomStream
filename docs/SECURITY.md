...
3. **Render chokepoints: `sanitizeFragment` and `sanitizeAttrValue`
   ...
   DOM fragment, and then imported. Attr ops are scrubbed before `setAttribute`. The viewer also
   runs a post-parse `sanitizeFragment` scrub on the mirror document after srcdoc loads.
