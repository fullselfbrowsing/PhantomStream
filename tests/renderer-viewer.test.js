...
// the locked handle shape {detach, destroy, getViewportMapping, registerOverlay, on}, snapshot
...
test('handle has exactly detach, destroy, getViewportMapping, registerOverlay, and on functions', () => {
  const env = setupEnv();
  try {
    const transport = createRecordingTransport();
    env.viewer = createViewer({
    ...
    assert.deepEqual(
      ...
      ['destroy', 'detach', 'getViewportMapping', 'on', 'registerOverlay'],
      'handle surface is locked to exactly five members'
    );
