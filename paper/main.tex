...
\definecolor{FSBInk}{HTML}{0F1117}
\definecolor{FSBPanel}{HTML}{1E1E2E}
\definecolor{FSBAmber}{HTML}{F59E0B}
\definecolor{FSBGreen}{HTML}{22C55E}
\definecolor{FSBMuted}{HTML}{888888}

\tikzset{
  psbox/.style={draw=FSBInk, line width=0.35pt, rounded corners=2pt,
               align=center, font=\scriptsize, text=FSBInk,
               text width=18mm, minimum height=9mm, inner sep=2pt,
               fill=FSBAmber!8},
  pswide/.style={psbox, text width=28mm},
  psaccent/.style={psbox, draw=FSBAmber!75!FSBInk, fill=FSBAmber!14},
  psgreen/.style={psbox, draw=FSBGreen!60!FSBInk, fill=FSBGreen!10},
  psorange/.style={psbox, draw=FSBAmber!75!FSBInk, fill=FSBAmber!14},
  psgray/.style={psbox, draw=FSBMuted!80!FSBInk, fill=FSBMuted!12},
  flow/.style={-{Latex[length=1.6mm,width=1.25mm]}, line width=0.45pt,
               draw=FSBMuted!80!FSBInk, shorten >=2pt, shorten <=2pt},
  accentflow/.style={flow, draw=FSBAmber!85!FSBInk},
  greenflow/.style={flow, draw=FSBGreen!70!FSBInk},
}
\pgfplotsset{
  fsbaxis/.style={
    axis line style={draw=FSBMuted!70},
    tick style={draw=FSBMuted!70},
    ticklabel style={font=\scriptsize, text=FSBInk},
    label style={font=\scriptsize, text=FSBInk},
    grid style={draw=FSBMuted!20},
    major grid style={draw=FSBMuted!24},
  },
  phantomplot/.style={draw=FSBAmber!85!FSBInk, fill=FSBAmber!28},
  rrwebplot/.style={draw=FSBGreen!65!FSBInk, fill=FSBGreen!18},
  cdpplot/.style={draw=FSBInk, fill=FSBMuted!28},
}
...
\begin{tikzpicture}[
  node distance=3mm,
  stage/.style={draw=FSBInk, rounded corners=2pt, align=center, font=\tiny,
    text=FSBInk, text width=11mm, minimum height=7mm, inner sep=1pt,
    fill=FSBAmber!8},
]
  \node[stage, draw=FSBMuted!80!FSBInk, fill=FSBMuted!12] (attach) {attach};
  \node[stage, right=of attach] (snapshot) {snapshot};
  \node[stage, draw=FSBGreen!60!FSBInk, fill=FSBGreen!10, right=of snapshot] (live) {live\\diffs};
  \node[stage, draw=FSBAmber!75!FSBInk, fill=FSBAmber!14, right=of live] (recover) {recover};
  \node[stage, right=of recover] (control) {control};
  \begin{scope}[on background layer]
    \draw[flow] (attach) -- (snapshot);
    \draw[accentflow] (snapshot) -- (live);
    \draw[accentflow] (live) -- (recover);
    \draw[flow] (recover.south west) to[out=-140,in=-40,looseness=0.8] (live.south east);
    \draw[accentflow] (live) -- (control);
  \end{scope}
\end{tikzpicture}
...
\begin{tikzpicture}[node distance=7mm and 8mm]
  ...
  \node[psaccent, text width=28mm, right=of relay] (viewer) {sandboxed\\viewer};
  \node[psgreen, below=of capture] (identity) {WeakMap\\node ids};
  \node[psgreen, below=of viewer] (control) {reverse\\control};
  \begin{scope}[on background layer]
    \draw[accentflow] (page) -- (capture);
    \draw[accentflow] (capture) -- (transport);
    \draw[flow] (transport) -- (relay);
    \draw[accentflow] (relay) -- (viewer);
    \draw[greenflow] (identity) -- (capture);
    \draw[greenflow] (viewer) -- (control);
    \draw[accentflow] (control.south west) .. controls +(down:24mm) and +(down:24mm) .. (page.south);
  \end{scope}
\end{tikzpicture}
...
\begin{tikzpicture}[node distance=2.6mm]
  ...
  \node[psaccent, text width=28mm, below=of runner] (systems) {PhantomStream / rrweb / CDP};
  ...
  \begin{scope}[on background layer]
    ...
    \draw[accentflow] (runner) -- (systems);
    \draw[accentflow] (systems) -- (json);
    \draw[flow] (json) -- (tex);
    \draw[greenflow] (tex) -- (paper);
    \draw[greenflow] (json.east) -- ++(8mm,0) |- (paper.east);
  \end{scope}
\end{tikzpicture}
...
\addplot[phantomplot, bar shift=0pt] coordinates
  {(PhantomStream,\PSPhantomMedianKB)};
\addplot[rrwebplot, bar shift=0pt] coordinates
  {(rrweb,\PSRrwebMedianKB)};
\addplot[cdpplot, bar shift=0pt] coordinates
  {(CDP,\PSCDPMedianKB)};
\end{axis}
\end{tikzpicture}
...
\addplot[phantomplot, bar shift=0pt] coordinates
  {(PhantomStream,\PSPhantomLatencyMs)};
\addplot[rrwebplot, bar shift=0pt] coordinates
  {(rrweb,\PSRrwebLatencyMs)};
\addplot[cdpplot, bar shift=0pt] coordinates
  {(CDP,\PSCDPLatencyMs)};
\end{axis}
\end{tikzpicture}
...
\begin{axis}[
  ...
  legend style={font=\scriptsize, at={(0.03,0.03)}, anchor=south west,
    draw=FSBMuted!55, fill=white},
]
\addplot[only marks, mark=*, mark size=2.4pt, color=FSBAmber!85!FSBInk,
  mark options={draw=FSBAmber!85!FSBInk, fill=FSBAmber!70}] coordinates {(\PSPhantomLatencyMs,\PSPhantomSemantic)};
\addlegendentry{PhantomStream}
\addplot[only marks, mark=square*, mark size=2.4pt, color=FSBGreen!65!FSBInk,
  mark options={draw=FSBGreen!65!FSBInk, fill=FSBGreen!55}] coordinates {(\PSRrwebLatencyMs,\PSRrwebSemantic)};
\addlegendentry{rrweb}
\addplot[only marks, mark=triangle*, mark size=2.8pt, color=FSBInk,
  mark options={draw=FSBInk, fill=FSBMuted!55}] coordinates {(\PSCDPLatencyMs,\PSCDPSemantic)};
\addlegendentry{CDP}
\end{axis}
\end{tikzpicture}
