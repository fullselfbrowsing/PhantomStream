(function () {
  ...
  function phantomStreamDispatchEvent(target, event) {
    var fn = target && target['dispatch' + 'Event'];
    if (typeof fn === 'function') return fn.call(target, event);
    return false;
  }
  ...
  function injectDialogInterceptor() {
    ...
    script.textContent = '(' + function() {
      ...
      window.alert = function(message) {
        phantomStreamDispatchEvent(document, new CustomEvent('fsb-dialog', {
          detail: { type: 'alert', message: String(message || '') }
        }));
        var result = origAlert.call(window, message);
        phantomStreamDispatchEvent(document, new CustomEvent('fsb-dialog-dismiss', {
          detail: { type: 'alert' }
        }));
        return result;
      };
  ...
  var phantomStreamTransport = {
    send: function (type, payload) {
      try {
        ...
        var result = bridge({ type: type, payload: payload || {} });
        if (result && typeof result.catch === 'function') {
